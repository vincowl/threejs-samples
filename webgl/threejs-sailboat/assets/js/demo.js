var DEMO = {
	...
	loadSailBoat: function loadSailBoat() {
		var axisHelper = new THREE.AxisHelper( 20 );
		this.ms_Scene.add( axisHelper );
		var loader = new THREE.JSONLoader();
		loader.load('../images/cad/namani/namani.json', function(geometry, materials) {
			DEMO.ms_SailBoat = new THREE.Mesh(geometry, new THREE.MeshFaceMaterial(materials));
			DEMO.ms_SailBoat.scale.x = DEMO.ms_SailBoat.scale.y = DEMO.ms_SailBoat.scale.z = 30;
			//DEMO.ms_SailBoat.translateZ(-1000);
			//DEMO.ms_SailBoat.translateX(-1000);
			DEMO.ms_SailBoat.translateY(-3); // Enfoncement du bateeau --> ligne de flottaison
			DEMO.ms_Scene.add(DEMO.ms_SailBoat);
		});
	},
	...
	updateSailBoat: function updateSailBoat(time) {
		if (this.ms_SailBoat != null) {
			this.ms_SailBoat.rotation.x = .06*Math.cos(time); // roulis
			this.ms_SailBoat.rotation.y = .03*Math.cos(time); // lacet
			this.ms_SailBoat.rotation.z = .01*Math.cos(time); // tangage
		}
	},
	...
	update: function update() {
		...
		this.updateSailBoat(this.ms_Water.material.uniforms.time.value);
